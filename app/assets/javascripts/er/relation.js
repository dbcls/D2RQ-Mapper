...
SQL.Relation = function(owner, row1, row2, join_id, color) {
    this.owner = owner;
    this.row1 = row1;
    this.row2 = row2;
    ...
    } else if (CONFIG.RELATION_COLORS) {
	// pick next
    ...
    */
	...
	path.setAttribute("data-joinid", this.join_id);
	/*
	$(path).click(function() {
	    var join_id = $(this).attr("data-joinid");
	    $.getScript("/ajax/er/table_join_dialog/" + join_id);
	});
	$(path).click(function() {
	    var join_id = $(this).attr("data-joinid");
	    $.getScript("/ajax/er/table_join_dialog/" + join_id);
