...
SQL.Row.prototype._build = function() {
    ...
    this.dom.container = OZ.DOM.elm("tbody", {className: "sqldesigner-column-row"});
...
SQL.Row.prototype.setTitle = function(t) {
    ...
    $(this.dom.container).attr('id', this.owner.getTitle() + "-" + t);
...
SQL.Row.prototype.dblclick0 = function(e) { /* dblclicked on row */
    OZ.Event.prevent(e);
    OZ.Event.stop(e);
    this.expand();
}
...
SQL.Row.prototype.redraw = function() {
    ...
    /* --> for D2RQ Mapper */
    //console.log(this.property_bridge_id);
    $(this.dom.container).attr('data-pbid', this.property_bridge_id);
    if (!this.enable) {
	$(this.dom.container).addClass('disable');
    } else {
	$(this.dom.container).removeClass('disable');
    }
...
SQL.Row.prototype.toXML = function() {
    ...
    xml += '<row name="' + t + '" id="' + this.property_bridge_id + '" enable="' + this.enable + '">\n';
...
SQL.Row.prototype.fromXML = function(node) {
    ...
    var property_bridge_id = node.getAttribute("id");
    ...
    this.property_bridge_id = property_bridge_id;
    this.enable = enable;
