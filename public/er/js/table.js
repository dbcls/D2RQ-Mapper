...
SQL.Table.prototype._build = function() {
    this.dom.container = OZ.DOM.elm("div", {className:"sqldesigner-table"});
    this.dom.content = OZ.DOM.elm("table");
    //-->var thead = OZ.DOM.elm("thead");
    this.dom.thead = OZ.DOM.elm("thead", {className: "sqldesigner-tablename"});
    //-->var tr = OZ.DOM.elm("tr");
    //-->var tr = OZ.DOM.elm("tr", {className: "sqldesigner-tablename"});
    this.dom.tr = OZ.DOM.elm("tr");
    this.dom.title = OZ.DOM.elm("td", {className: "sqldesigner-title", colSpan: 2});
    
    OZ.DOM.append(
	[this.dom.container, this.dom.content],
	[this.dom.content, this.dom.thead],
	[this.dom.thead, this.dom.tr],
	[this.dom.tr, this.dom.title]
    );
    
    this.dom.mini = OZ.DOM.elm("div", {className:"sqldesigner-mini"});
...
SQL.Table.prototype.dblclick0 = function(e) {
    var t = OZ.Event.target(e);
    if (t == this.dom.title) {
...
SQL.Table.prototype.redraw = function() {
    ...
    /* --> for D2RQ Mapper */
    $(this.dom.thead).attr('data-cmid', this.class_map_id);
    if (!this.enable) {
	$(this.dom.thead).addClass('disable');
    } else {
	$(this.dom.thead).removeClass('disable');
    }
...
SQL.Table.prototype.toXML = function() {
    ...
    var xml = "";
    xml += '<table id="' + this.class_map_id + '" enable="' + this.enable + '" x="' + this.x + '" y="' + this.y + '" name="' + t + '">\n';
    for (var i = 0; i < this.rows.length; i++) {
...
SQL.Table.prototype.fromXML = function(node) {
    ...
    var class_map_id = node.getAttribute("id");
